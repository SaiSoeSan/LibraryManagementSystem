...
public class TestCheckoutBook {
    public static void main(String[] args) {
        try {
            Address address = Address.CreateNewAddress("Street 1", "Fairfield", "IA", "52556;");
            Member member = Member.createMember("Abdul", "Moiz", "641222222", address, "1");
            DataAccess.saveMember(member);

            Author author = new Author("James", "Clear", "64199", address, "lets make a difference", "credential");
            List<Author> authors = new ArrayList<Author>();
            authors.add(author);
            Book book = Book.createNewBook("1100452", "Atomic Habits", 10, authors, 5);
